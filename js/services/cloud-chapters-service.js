/**
 * Capítulos em armazenamento remoto — índice e páginas via API Cloudflare (R2).
 */
...
function paginasDiretasDoIndice(info) {
    ...
    return hosted.map((p, i) => ({
        index: p.index ?? i,
        url: p.url,
        origem: p.origem || (String(p.url).includes("telegra.ph") ? "telegra" : String(p.url).includes("catbox.moe") ? "catbox" : "r2-api")
    }));
}
...
export async function obterPaginasRemotas(mangaId, capId) {
    ...
    const usesApi = telegraPages?.some((p) => String(p.url).includes("/api/cloud/page"));

    if (telegraPages?.length && !usesApi) return telegraPages;
...
/** Caps prontos para leitura (API R2, Telegra.ph ou backup local em dev). */
