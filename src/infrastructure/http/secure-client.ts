...
function sameOriginRedirect(baseUrl: string, location: string): boolean {
    try {
...
export async function fetchText(url: string, opts: {
    referer?: string;
    ...
    for (let hop = 0; hop <= maxHops; hop++) {
        ...
        const res = await client.get<string>(current, config);
        ...
        if (res.status >= 300 && res.status < 400 && res.headers.location) {
            ...
            if (hop >= maxHops) throw new Error("Muitos redirects.");
