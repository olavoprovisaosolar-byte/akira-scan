...
export function resolveAbsoluteUrl(base: string, path: string): string {
    const p = (path || "").trim();
    if (!p) return "";
...
export function isValidChapterImageUrl(url: string): boolean {
    if (!url || typeof url !== "string") return false;
    const u = url.toLowerCase();
    if (u.includes("netassets")) return false;
    if (/logo|avatar|banner|icon|favicon|placeholder|placehold\.co/i.test(u)) return false;
    if (/\/covers?\//i.test(u) && !/\/uploads\/.*chapter/i.test(u)) return false;
    if (/\d{2,3}x\d{2,3}\.(webp|jpg|png)/i.test(u)) return false;
    return /\.(webp|jpg|jpeg|png|gif)(\?|$)/i.test(u) || u.includes("/uploads/");
}

export function validateChapterPages(pages: { url?: string }[]): boolean {
