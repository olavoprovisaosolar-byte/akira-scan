...
function readIngestionStatus(): Record<string, unknown> | null {
    try {
        if (!fs.existsSync(INGESTION_STATUS_PATH)) return null;
        return JSON.parse(fs.readFileSync(INGESTION_STATUS_PATH, "utf8")) as Record<string, unknown>;
    } catch {
        return null;
    }
}
...
function clientHeadersFromReq(req: Request): Record<string, string> {
...
export async function handleProxyRequest(req: Request): Promise<Response> {
    ...
    try {
        ...
        if (mangaMatch) {
            ...
            const cached = await cacheGet<MangaCanonical>(key);
            if (cached) {
                return json({
        ...
        if (capMatch) {
            ...
            const cached = await cacheGet<PageRef[]>(key);
            if (cached) {
                return json({
                    manga: mangaId,
                    capitulo: chapterId,
        ...
        return json({
            error: (error as Error).message || "Erro interno no proxy.",
            cached: false
        }, 502);
    }
}
