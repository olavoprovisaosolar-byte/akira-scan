...
function resolveOrder(preferred: string, mangaId: string): ProviderName[] {
    if (preferred !== "auto") {
        const p = preferred as ProviderName;
        if (scrapers[p]) return [p, ...PROVIDER_ORDER.filter((x) => x !== p)];
        return [...PROVIDER_ORDER];
    }
    if (/^obra-/i.test(mangaId)) return ["toonlivre", "mangalivre", "mangalivreto", "mangalivreblog", "bladetoons"];
    return ["mangalivreto", "mangalivreblog", "toonlivre", "mangalivre", "bladetoons"];
}

function validateMangaPayload(manga: MangaLegacy | null | undefined): { ok: boolean; error?: string } {
...
function isValidMangaData(manga: MangaLegacy | null | undefined): boolean {
    return validateMangaPayload(manga).ok;
}

export async function fetchMangaWithFailover(
    mangaId: string,
    preferred = "auto"
    ...
    throw new Error(
        attempts.map((a) => `${a.provider}: ${a.error || "vazio"}`).join(" | ")
            || "Nenhum provedor disponível."
    );
}

export async function fetchMangaAuto(
    mangaId: string,
    preferred = "auto"
): Promise<{ manga: MangaLegacy; source: string }> {
    const { manga, source } = await fetchMangaWithFailover(mangaId, preferred);
    return { manga, source };
}

export async function fetchChapterAuto(
    mangaId: string,
    chapterId: string,
    numeroCap: string,
    preferred = "auto",
    clientHeaders: Record<string, string> = {}
): Promise<{ pages: PageRef[]; source: string }> {
    const order = resolveOrder(preferred, mangaId);
    const errors: string[] = [];
    ...
    for (const name of order) {
        ...
        try {
            ...
            logger.warn("ScraperRegistry", `${name} capítulo falhou`, { mangaId, chapterId, msg: (e as Error).message });
        }
    }
