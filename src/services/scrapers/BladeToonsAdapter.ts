...
export class BladeToonsScraper {
    ...
    async fetchManga(mangaId: string): Promise<MangaLegacy> {
        ...
        return getBreaker(this.name).exec(() =>
            withRetry(() => this.scrapeManga(mangaId), { label: `bladetoons:manga:${mangaId}` })
        );
    }
    ...
    private async loadHtml(pathOrUrl: string): Promise<string> {
    ...
    private async scrapeManga(mangaId: string): Promise<MangaLegacy> {
        ...
        if (!html) throw new Error(`BladeToons — mangá não encontrado: ${slug}`);
        ...
        const mangaData = pageProps?.manga as Record<string, unknown> | undefined;

        const $ = cheerio.load(html);
        const titulo = String(
            pageProps?.title || mangaData?.title || $("h1").first().text().trim()
        ...
        if (Array.isArray(apiChapters) && apiChapters.length) {
            capitulos = (apiChapters as Array<Record<string, unknown>>).map((c, i) => {
            ...
            try {
                ...
                if (!v.ok) throw new Error(v.error || "Capítulos BladeToons inválidos.");
    ...
    private async scrapeChapter(mangaId: string, numeroCap: string): Promise<string[]> {
        ...
        if (!html) throw new Error("Capítulo BladeToons não encontrado.");
        ...
        $(BLADETOONS_SELECTORS.pageImages).each((_, el) => {
            ...
            if (!src || /logo|avatar|icon/i.test(src)) return;
        ...
        if (!urls.length) throw new Error("BladeToons — nenhuma imagem no capítulo.");
        return urls;
    }
