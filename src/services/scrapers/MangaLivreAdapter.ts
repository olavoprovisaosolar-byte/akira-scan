...
export class MangaLivreScraper {
    ...
    async fetchManga(mangaId: string): Promise<MangaLegacy> {
        return getBreaker(this.name).exec(() =>
            withRetry(() => this.scrapeManga(mangaId), { label: `mangalivre:manga:${mangaId}` })
        );
    }

    async fetchChapterPages(mangaId: string, _chapterId: string, numeroCap: string): Promise<string[]> {
        return getBreaker(this.name).exec(() =>
    ...
    private async scrapeChapter(mangaId: string, numeroCap: string): Promise<string[]> {
        ...
        if (!html) throw new Error("Capítulo MangaLivre não encontrado.");
