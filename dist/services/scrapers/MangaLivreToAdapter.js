...
export class MangaLivreToScraper {
    ...
    async fetchManga(mangaId) {
        ...
        return getBreaker(this.name).exec(() => withRetry(() => this.scrapeManga(mangaId), { label: `mangalivreto:manga:${mangaId}` }));
    }
    ...
    async listCatalogSlugs(maxPages = 3) {
        const slugs = new Set();
    ...
    async scrapeChapter(mangaId, numeroCap) {
        ...
        if (!html)
            throw new Error("Capítulo MangaLivreTo não encontrado.");
        ...
        if (urls.length < 2 && usePlaywright()) {
            ...
            const scrollHtml = await fetchHtmlWithBrowser(`${BASE}/manga/${encodeURIComponent(slug)}/capitulo-${numeroCap}/`, 4000);
