...
export function isPlaceholderCover(url) {
    if (!url || url === MANGA_FALLBACKS.coverUrl)
        return true;
...
export async function checkCoverAccessible(coverUrl) {
    ...
    try {
        ...
        try {
            ...
            return res.status < 400 && res.data.byteLength > 100;
        }
        catch {
            return false;
        }
...
export async function checkChapterAccessible(chapterUrl) {
    if (!chapterUrl)
        return false;
    try {
...
export async function runSanityCheck(manga) {
    const errors = [];
    const warnings = [];
